...
class TestProcessor(BaseFlaskTest):
    ...
    def setUpClass(self):
        ...
        with open(
            ...
            self.expected_processor = json.load(f)

    @mock.patch.object(processor, 'g')
    def test_get_processor_server_hardware_not_found(self, g):
        g.oneview_client.server_hardware.get.side_effect = \
            HPOneViewException({"errorCode": "RESOURCE_NOT_FOUND"})

        response = self.client.get(
            "/redfish/v1/CompositionService/ResourceBlocks"
            "/30303437-3034-4D32-3230-313133364752/Systems/1/Processors/1")

        self.assertEqual(status.HTTP_404_NOT_FOUND, response.status_code)
        self.assertEqual("application/json", response.mimetype)

    @mock.patch.object(processor, 'g')
    def test_get_processor_id_not_found(self, g):
        g.oneview_client.server_hardware.get.return_value = \
            self.server_hardware

        response = self.client.get(
            "/redfish/v1/CompositionService/ResourceBlocks"
            "/30303437-3034-4D32-3230-313133364752/Systems/1/Processors/999")
    ...
    def test_get_processor(self, g):
        ...
        self.assertEqual("application/json", response.mimetype)
        self.assertEqualMockup(self.expected_processor, result)
