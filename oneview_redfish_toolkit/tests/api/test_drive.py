...
class TestDrive(BaseTest):
    ...
    @classmethod
    def setUpClass(self):
        super(TestDrive, self).setUpClass()
        ...
        with open(
            ...
            self.drive_resource_block = json.load(f)
    ...
    def test_build_for_resource_block(self):
        ...
        drive = Drive.build_for_resource_block(self.drive,
                                               self.drive_enclosure)

        result = json.loads(drive.serialize())

        self.assertEqualMockup(self.drive_resource_block, result)
