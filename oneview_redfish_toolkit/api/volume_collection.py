...
class VolumeCollection(RedfishJsonValidator):
    ...
    def __init__(self, server_profile):
        ...
        self.redfish["Members@odata.count"] = len(server_profile["localStorage"]["sasLogicalJBODs"])
        self.redfish["Members"] = list()
        
        
        for i in range(len(server_profile["localStorage"]["sasLogicalJBODs"])):
            link_dict = collections.OrderedDict()            
            link_dict["@odata.id"] = "/redfish/v1/Systems/" + server_profile["uuid"] + "/Storage/1/Volumes/"+str(server_profile["localStorage"]["sasLogicalJBODs"][i]["id"])
        ...
        self.redfish["@odata.context"] = \
            ...
            ".VolumeCollection"
        self.redfish["@odata.id"] = "/redfish/v1/Systems/" + server_profile["uuid"] + "/Storage/1/Volumes"
        self._validate()
