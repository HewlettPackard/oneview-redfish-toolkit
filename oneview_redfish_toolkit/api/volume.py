...
class Volume(RedfishJsonValidator):
    ...
    def build_volume_details(uuid, volume_id):
        ...
        sas_logical_jbod = get_sas_logical_jbod_by_volumeid(server_profile,volume_id)
            
        if sas_logical_jbod is None:
            abort(status.HTTP_404_NOT_FOUND, "Volume {} not found"
                  .format(volume_id))
            
        sas_Logical_Interconnect_Uri =  sas_logical_jbod["sasLogicalInterconnectUri"]
        
        drivepaths = []
        drivepath = None             
        for logical_Drive_Bay_Uri in sas_logical_jbod["logicalDriveBayUris"]:
            drivepath =  get_drive_path_from_logical_Drive_Bay_Uri(logical_Drive_Bay_Uri)
            drivepaths.append(drivepath)
            
        drive_enclosure_uri = get_drive_enclosure_uri_from_sas_Logical_Interconnect(sas_Logical_Interconnect_Uri)
        
        drive_enclosure_object = get_drive_enclosure_object(drive_enclosure_uri)
        ...
        device_slot = get_device_slot_from_sas_logical_jbod_by_volumeid(server_profile,volume_id)                       
        raidlevel = None
        flag = False
        ...
        attrs = {}         
        attrs["@odata.id"] = ComputerSystem.BASE_URI + "/"+ uuid + "/Storage/1/Volumes/" + volume_id
        attrs["Id"] = volume_id
        ...
        if not raidlevel is None :
            attrs["VolumeType"] = status_mapping.RAID_LEVEL.get(raidlevel)
        else:
            attrs["VolumeType"] = "RawDevice"
        attrs["CapacityBytes"] = get_capacity_in_bytes(sas_logical_jbod["maxSizeGB"])
        ...
        for drivebayuri in drivebayuris:
            attrs["Links"]["Drives"].append(
                {
                    "@odata.id": ComputerSystem.BASE_URI + "/"+ uuid + "/Storage/1/Drives/"+ drivebayuri.split("/")[-1]
                }
            )
...
def get_sas_logical_jbod_by_volumeid(server_profile,volume_id):
    ...
    for logical_jbod in server_profile["localStorage"]["sasLogicalJBODs"]:
        if logical_jbod["id"] == int(volume_id) and logical_jbod["sasLogicalJBODUri"]:
...
def get_device_slot_from_sas_logical_jbod_by_volumeid(server_profile,volume_id):
...
def get_drive_enclosure_uri_from_sas_Logical_Interconnect(sas_Logical_Interconnect_Uri):
    item = g.oneview_client.sas_logical_interconnects.get(sas_Logical_Interconnect_Uri)
